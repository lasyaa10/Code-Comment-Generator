...
function activate(context) {
  let disposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      let apiKey = context.globalState.get("grokApiKey");
      if (!apiKey) {
        ...
        await context.globalState.update("grokApiKey", apiKey);
      }
      try {
        const response = await axios_default.post(
          "https://api.x.ai/v1/chat/completions",
          {
            ...
            messages: [
              {
                role: "system",
                content: "You are a code documentation assistant. Generate concise, accurate comments for the provided code in the language's standard format (e.g., JSDoc for JavaScript, docstrings for Python). Include function descriptions, parameters, and return values where applicable."
              },
              {
                role: "user",
                content: `Generate comments for this code:
...
${code}`
        ...
        const generatedComments = response.data.choices[0].message.content;
        await editor.edit((editBuilder) => {
          const position = editor.selection.start;
          editBuilder.insert(position, `${generatedComments}
`);
        });
        ...
        vscode.window.showErrorMessage(
          `Failed to generate comments: ${errorMessage}`
        );
